...
- **Releases follow one ritual.** Update `CHANGELOG.md`, run `npm version <major|minor|patch>`, `git push --follow-tags`. The `release.yml` workflow builds, tests, stages the npm publish through trusted publishing and creates the GitHub Release; Mark approves the staged version on npmjs.com; then run the `verify-published` workflow with the version. Never publish from a laptop with a token once trusted publishing is configured.
...
npm run test:consumers # pack, install the tarball into a scratch project, run the ESM, CJS, type and bin fixtures
