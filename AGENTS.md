...
- **Dependencies.** Dependabot opens weekly PRs (npm and GitHub Actions). Merge when CI is green; majors get a look at the release notes first. Actions are pinned to commit SHAs with the version in a comment; keep it that way. Dependency install scripts stay off: when npm warns that a package's install script is not covered by `allowScripts`, check that lint, typecheck and the build pass after `npm ci --ignore-scripts`, then add the package to `allowScripts` in `package.json` as `false` (unrs-resolver's native-binary fallback is there since 2026-09-25; npm 12 blocks these scripts by default).
...
- Node's `TextDecoder` decodes windows-1252 (and latin1, iso-8859-1, us-ascii) bytes 0x80 to 0x9F as C1 controls on Node 20 (from 20.18.3), 22.13.0 to 22.22.0 and 24.0.0 to 24.13.0 (fixed in 22.22.1, 24.13.1 and 25.4.0), so the library decodes that encoding family itself. Do not hand it back to `TextDecoder`.
