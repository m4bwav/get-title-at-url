...
- [x] Mark: uninstall Snyk from GitHub (github.com/settings/installations, then remove the project at app.snyk.io) so it stops opening PRs. The `gh` token cannot list app installations (verified: HTTP 403), so this is a browser task. (2026-09-25: Mark revoked Snyk under Authorized OAuth Apps and found no Snyk GitHub App; the agent swept the repository (webhooks, deploy keys, secrets, code scanning, collaborators, check runs) and found nothing Snyk, see the log. A project left at app.snyk.io can no longer reach the repository.)
...
- [x] Mark: confirm the npm account has two-factor authentication on (npmjs.com, Account, Two-Factor Authentication). Trusted publishing itself is configured in Stage 3, after the workflow file exists. (2026-09-25: shown by Mark approving 3.0.0-beta.1, which npm allows only with 2FA.)
...
- [x] Agent: rehearsal. `npm version 3.0.0-beta.1` on `master`, `git push --follow-tags`; the workflow stages the version under `next`; Mark approves it on npmjs.com. Verify with `npm view get-title-at-url@next version`, `npm view get-title-at-url dist-tags` (`latest` must still be `2.0.0`), `npx -y get-title-at-url@next https://example.com/` on this machine, and `npm audit signatures` in a temp project. This is also where to confirm that staged publishing honours `--tag next` (unverified in the research; if it does not, a prerelease without the tag would become `latest`, so publish prereleases directly or skip the rehearsal and rely on the CI consumer fixtures). Fix and repeat with `beta.2` if anything fails. (2026-09-25: `npm version 3.0.0-beta.1` committed e26bf22 and tag v3.0.0-beta.1; release run 36153096874 green: build job 186 of 186 tests, publint and attw clean, consumers 7 pass and 5 skipped, tarball 40.2 kB with 11 files; `npm stage publish` staged it "with tag next", stage id f6123f79-4603-46ce-91b8-23d82362cb18, provenance signed (sigstore log index 2957911315); GitHub prerelease v3.0.0-beta.1 created; `latest` still 2.0.0. Mark approved it; the approval kept `next` (`latest` still 2.0.0); `npx -y get-title-at-url@next` printed Example Domain; `npm audit signatures` verified 1 signature and 1 attestation. verify-published run 36154884494 failed only in Deno's `deno run npm:` step (Deno 2.9's 24-hour minimum dependency age), fixed in 8550db9; run 36155346459 green in all 15 jobs.)
- [x] Agent: set the changelog date, `npm version 3.0.0`, `git push --follow-tags`, watch the run; Mark approves the staged 3.0.0 on npmjs.com; agent runs `verify-published.yml` with `3.0.0`. Verify: `npm view get-title-at-url version` prints `3.0.0`; `npx -y get-title-at-url@3 https://example.com/` prints `Example Domain`; a fresh temp project can `import` and `require` it on Node 20, 22 and 24 (the CI smoke job repeats this against the registry); the GitHub Release for `v3.0.0` exists with notes; the npm page shows provenance. (2026-09-25: CHANGELOG dated 4ddde16, `npm version 3.0.0` be8eb1b, tag v3.0.0; release run 36155632874 green and staged 3.0.0 "with tag latest", stage id d5077ead-4a84-4a01-a8e7-639baa371096, provenance signed; GitHub Release v3.0.0 created. Mark approved it: `npm view get-title-at-url version` 3.0.0, dist-tags latest 3.0.0 and next 3.0.0-beta.1, no dependencies, SLSA provenance v1; `npx -y get-title-at-url@3 https://example.com/` printed Example Domain and `--version` 3.0.0; a temp project on Node 24.18.0 installed 1 package with 0 vulnerabilities, `npm audit signatures` verified 1 signature and 1 attestation, `require` and `import` both worked; `gh release view v3.0.0` shows the changelog section, not a prerelease; verify-published run 36156488030 green in all 15 jobs, so Node 20 and 22 import and require it from the registry too.)
- [x] Agent: close issue #6 with a note that 3.0.0 has no runtime dependencies and the warnings came from the old dev tools. Update the README badges if any URL changed. (2026-09-25: closed with a comment that follows the issue table below, since the warnings came from `request` in 1.1.8 rather than the dev tools: issuecomment-5835268349. The v3 README badges needed no change; the registry now serves the v3 README, so the npm page lost 2.0.0's dead badges.)
- [x] Agent: log the release, rewrite `ai-docs/HANDOFF.md`, and record the trusted-publishing steps as a solution entry once they are verified. (2026-09-25: [../solutions/2026-09-25-publish-to-npm-from-github-actions-without-a-stored-token-th.md](../solutions/2026-09-25-publish-to-npm-from-github-actions-without-a-stored-token-th.md); HANDOFF.md rewritten around Stage 4.)
...
- [ ] Optional Stage 5, JSR half: publish to JSR from the same source (`jsr.json`, `npx jsr publish` with OIDC in `release.yml`). Advised against on 2026-09-25 (HANDOFF item 6).
- [x] Optional Stage 5, tool-page half: a small "title of a URL" tool page on markdavidrogers.com using the same package (the site plan's Stage 3 tools area). (2026-09-25: live on the Azure preview's `/tools`, markdavidrogers-web PR #12, merge 77a453b, Deploy run 36174106424. The server fetches behind SSRF guards and the island runs 3.0.0 with an injected `fetch`. Verified live: example.com gives "Example Domain", a Shift_JIS page declared only in a meta tag gives "太宰治 走れメロス", and 169.254.169.254 and localhost are refused. Design in the web repo's `ai-docs/decisions/2026-09-25-title-of-a-url-tool.md`.)
...
| Other runtimes | `bunx get-title-at-url https://example.com/` and `deno run -A npm:get-title-at-url https://example.com/` (manual, once; within 24 hours of a publish Deno 2.9+ needs `--minimum-dependency-age=0`) | `Example Domain` |
...
- Staged publishing and `--tag next`: npm refuses to stage a prerelease without `--tag` (npm 11.19 source, 2026-09-25), so a beta cannot become `latest` at staging; the approval keeps the staged tag (rehearsal, 2026-09-25: after approving 3.0.0-beta.1, `latest` stayed 2.0.0 and `next` became 3.0.0-beta.1).
...
None pending for the release. Stage 4 is standing work: merge Dependabot pull requests when CI is green (the first arrive on a Monday), and plan v4 before Node 22 reaches end of life on 2027-04-30.
