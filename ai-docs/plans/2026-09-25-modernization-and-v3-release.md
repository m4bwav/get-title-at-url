...
Mark adds the trusted publisher on npmjs.com (the fields are in Stage 3's first item) and says so; then the agent runs the rehearsal on `master`: `npm version 3.0.0-beta.1`, `git push --follow-tags`, and watches `release.yml` stage the version under `next` for Mark's approval.
