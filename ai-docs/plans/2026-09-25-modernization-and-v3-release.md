...
Mark approves the staged 3.0.0 on npmjs.com (Staged Packages tab, 2FA, stage id d5077ead) and says so; then the agent runs the checks in Stage 3's third item, `verify-published.yml` with `3.0.0`, closes issue #6, and rewrites HANDOFF.md around Stage 4.
