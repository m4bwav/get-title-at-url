...
- [ ] Branch `v3` from `master`. Remove `.travis.yml`, `.snyk`, `index.js`, `cli.js`, `test.js`, `package-lock.json`.
- [ ] `package.json` as in the target table. Keep `"version": "2.0.0"` until the release step bumps it. Scripts: `build` (tsdown), `test` (build, then `node --test`), `test:live`, `lint` (xo), `typecheck` (tsc --noEmit), `check` (publint, attw, `npm pack --dry-run`), `prepack` (build), `prepublishOnly` (lint, typecheck, check, test).
- [ ] `tsconfig.json` on a the tsconfig node20 base package base with `module` and `moduleResolution` `nodenext`, `verbatimModuleSyntax`, `strict`, `declaration` off (tsdown emits types). `tsdown.config.ts` with two entries, `format: ['esm', 'cjs']`, `dts: true`, the CLI entry marked so its shebang survives.
- [ ] src/extract-title.ts per D2, with the entity table and the separator rules as named constants and a comment citing article-title (MIT, sindresorhus) for the heuristic's origin.
- [ ] src/index.ts per D3 and D8. src/errors.ts for `GetTitleError`. src/cli.ts with `parseArgs` per the API section and D17.
- [ ] `test/` per the test strategy below: layers 1 to 5 and 8 land in this stage (unit, functional, CLI, package shape, consumer fixtures, live smoke); layers 6, 7 and 9 are wired in Stage 2.
- [ ] `xo` config, `.editorconfig`, `.gitattributes` (`* text=auto eol=lf`), `.gitignore` (`dist`, `coverage`, `node_modules`), No `.npmrc` with `ignore-scripts=true`: it would also skip this package's own `prepack`. The supply-chain guards are the lockfile, Dependabot's cooldown (3 days by default since 2026-07-14) and npm 12's default block on dependency install scripts (npm 12.1.0 is `latest`; this machine has 11.16.0, upgrade when convenient).
- [ ] README rewrite: three live badges at most (npm version, CI, npm downloads), install, usage for ESM, CommonJS, TypeScript, Deno (`npm:get-title-at-url`), Bun, and the browser caveat (CORS; `extractTitle` works everywhere), API reference, error codes, CLI, migration from 2.x, license.
- [ ] `CHANGELOG.md` (Keep a Changelog format) with the 3.0.0 section and a compressed history of 1.x and 2.0.0 taken from the tags.
- [ ] `SECURITY.md` pointing at GitHub private vulnerability reporting.
- [ ] Local verification on Node 24: `npm run lint`, `npm run typecheck`, `npm run build`, `npm test`, `npm run check`, `npm pack --dry-run` lists `dist/*`, `README.md`, `LICENSE`, `CHANGELOG.md`, `package.json` and nothing else. Record the output in the log.
- [ ] Open the pull request `v3` into `master` so Stage 2's CI runs on it; squash-merge when green.

## Stage 2: GitHub setup (in the same pull request, 2026-09-26 to 2026-10-03)

- [ ] .github/workflows/ci.yml per the target table. Triggers: push to `master`, pull requests, manual. `concurrency` cancels superseded runs. Actions and their current majors: `actions/checkout` v7.0.1, `actions/setup-node` v7.0.0, `actions/upload-artifact` v7.0.1, `oven-sh/setup-bun` v2.2.0, `denoland/setup-deno` v2.0.5 (all checked 2026-09-24). Every one pinned to a commit SHA with the version in a comment; Dependabot updates both. Nothing older: runners removed the Node 20 action runtime on 2026-09-23.
- [ ] .github/workflows/release.yml per the target table. Trigger: tag `v*`. `permissions` with `id-token` set to `write` and `contents` set to `write`. Node 24 (its npm 11.19 satisfies the 11.5.1 floor for trusted publishing and the 11.15 floor for `npm stage publish`; Node 22 ships npm 10.9.9, which cannot do either). Guard step: the tag must equal `package.json` `version`. No `--provenance` flag and no token: both are automatic with OIDC. `repository.url` must match the GitHub repository exactly; the current `https://github.com/m4bwav/get-title-at-url.git` does. Prerelease versions (`-beta.N`) are staged with `--tag next`, releases with `latest`. Then `gh release create --verify-tag` with the matching changelog section (prereleases marked as such).
- [ ] .github/workflows/verify-published.yml (manual, version input) and .github/workflows/live.yml (weekly schedule and manual) per the test strategy.
- [ ] .github/dependabot.yml: npm weekly (Monday), groups `minor-and-patch`; github-actions weekly.
- [ ] `AGENTS.md` (what it is, rules, commands, the everlast block), `CLAUDE.md` (its first line imports AGENTS.md), `.github/copilot-instructions.md` (one-line pointer). Use the evergreen templates at `D:\m4bwa\Claude\Projects\Ai\evergreen\templates\`.
- [ ] Ruleset on `master`: block force pushes and deletion; require the `ci` status checks for pull requests; repository admin on the bypass list so a direct push by Mark still works.
- [ ] Verify: CI green on the PR (record the run id), attw and publint clean, the pack-and-install smoke job imports and requires the tarball on Node 20 and 24.

## Stage 3: publish 3.0.0 (2026-10-03 to 2026-10-10)

- [ ] Mark (one-time, browser): on npmjs.com open the package, Settings, Trusted publishing, add a GitHub Actions publisher: Organization or user `m4bwav`, Repository `get-title-at-url`, Workflow filename `release.yml`, Environment name blank, Allowed actions `npm stage publish` (the default; tick `npm publish` only if D16 is overruled). Entries cannot be edited, only deleted and recreated. Once it works, set the package to "Require two-factor authentication and disallow tokens". No token is stored anywhere.
- [ ] Agent: rehearsal. `npm version 3.0.0-beta.1` on `master`, `git push --follow-tags`; the workflow stages the version under `next`; Mark approves it on npmjs.com. Verify with `npm view get-title-at-url@next version`, `npm view get-title-at-url dist-tags` (`latest` must still be `2.0.0`), `npx -y get-title-at-url@next https://example.com/` on this machine, and `npm audit signatures` in a temp project. This is also where to confirm that staged publishing honours `--tag next` (unverified in the research; if it does not, a prerelease without the tag would become `latest`, so publish prereleases directly or skip the rehearsal and rely on the CI consumer fixtures). Fix and repeat with `beta.2` if anything fails.
...
| CJS works | `node -e "const g=require('get-title-at-url'); console.log(typeof g, typeof g.getTitleAtUrl)"` | `function function` |
...
- Staged publishing and `--tag next`: unverified together; see the rehearsal step.
...
| 9. Post-publish | The registry artifact, in every environment | `verify-published.yml` (manual with a version input, and called by `release.yml` after publishing, with a short wait for registry propagation): on Ubuntu, Windows and macOS with Node 20, 22, 24 and 26, plus Bun and Deno (`deno run -A npm:get-title-at-url@<version>`), install `get-title-at-url@<version>` from npmjs.com, run the consumer fixtures against it, run `npx get-title-at-url https://example.com/`, and run `npm audit signatures` so the provenance attestation is verified, not just displayed | After every publish |
...
Mark reads the decisions table (D1 to D17) and says what to change; silence keeps the recommendations. Then the agent runs the Stage 0 commands from the appendix and opens branch `v3` for Stage 1.
